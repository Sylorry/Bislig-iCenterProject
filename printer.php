...
try {
    ...
    // Fetch distinct model brands for Printer
    $modelStmt = $conn->prepare("SELECT DISTINCT LOWER(model) AS model FROM products WHERE LOWER(product) = 'Printer' ORDER BY model ASC");
    $modelStmt->execute();
    $modelBrands = $modelStmt->fetchAll(PDO::FETCH_COLUMN);

    // Fetch distinct storage options for Printer that have products
    $storageStmt = $conn->prepare("SELECT DISTINCT LOWER(storage) AS storage FROM products WHERE LOWER(product) = 'printer' ORDER BY storage ASC");
    $storageStmt->execute();
    $storageOptions = $storageStmt->fetchAll(PDO::FETCH_COLUMN);

    // Fetch all printer products for dropdowns
    $allProductsStmt = $conn->prepare("SELECT * FROM products WHERE LOWER(product) = 'printer' ORDER BY product_id ASC");
    $allProductsStmt->execute();
    $allProducts = $allProductsStmt->fetchAll(PDO::FETCH_ASSOC);

    // Build count query with filters
    $countQuery = "SELECT COUNT(*) FROM products WHERE LOWER(product) = 'printer'";
    $countParams = [];
    ...
    $selectQuery = "SELECT * FROM products WHERE LOWER(product) = 'printer'";
    $selectParams = [];
    if ($modelBrandFilter !== '' && $modelBrandFilter !== 'all models') {
    ...
    echo '<div class="text-red-500">Error loading printer products: ' . htmlspecialchars($e->getMessage()) . '</div>';
    $products = [];
    $totalPages = 0;
}
?>
...
<head>
  ...
  <title>Printer Products - Inventory System</title>
...
<body>
  <header class="flex flex-col py-4">
    ...
    <h1 class="text-6xl font-bold text-black-400 text-center" style="font-family: 'Times New Roman', serif;">PRINTER</h1>
  </header>
  ...
  <main class="p-8">
    ...
    <div class="grid">
      <?php
      if (count($products) === 0) {
          echo '<p class="col-span-full text-center text-gray-500 text-xl">No printer products found.</p>';
      } else {
