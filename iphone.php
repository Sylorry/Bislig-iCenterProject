...
// Fetch distinct model brands for iPhones
$modelStmt = $conn->prepare("SELECT DISTINCT LOWER(model) AS model FROM products WHERE LOWER(product) = 'iphone' ORDER BY model ASC");
$modelStmt->execute();
$modelBrands = $modelStmt->fetchAll(PDO::FETCH_COLUMN);

// Add Iphone 11 series, Iphone 12 series, Iphone 13 series and Iphone 14 series as special filter options
$specialSeriesNames = ['iphone 7 series', 'iphone 8 series', 'iphone 11 series', 'iphone 12 series', 'iphone 13 series', 'iphone 14 series', 'iphone 16 series', 'iphone x series'];
foreach ($specialSeriesNames as $specialSeriesName) {
    if (!in_array($specialSeriesName, $modelBrands)) {
        array_unshift($modelBrands, $specialSeriesName);
    }
}

    // Fetch distinct storage options for iPhones that have products
    $storageStmt = $conn->prepare("SELECT DISTINCT LOWER(storage) AS storage FROM products WHERE LOWER(product) = 'iphone' ORDER BY storage ASC");
    $storageStmt->execute();
    $storageOptions = $storageStmt->fetchAll(PDO::FETCH_COLUMN);

    // Fetch all iPhone products for dropdowns
    $allProductsStmt = $conn->prepare("SELECT * FROM products WHERE LOWER(product) = 'iphone' ORDER BY product_id ASC");
    $allProductsStmt->execute();
    $allProducts = $allProductsStmt->fetchAll(PDO::FETCH_ASSOC);

// Build count query with filters
$countQuery = "SELECT COUNT(*) FROM products WHERE LOWER(product) = 'iphone'";
$countParams = [];
if ($modelBrandFilter !== '' && $modelBrandFilter !== 'all models') {
    if ($modelBrandFilter === 'iphone 7 series') {
        // Count products with model in 7, 7p
        $countQuery .= " AND LOWER(model) IN ('7', '7p')";
    } elseif ($modelBrandFilter === 'iphone 8 series') {
        // Count products with model in 8, 8p
        $countQuery .= " AND LOWER(model) IN ('8', '8p')";
    } elseif ($modelBrandFilter === 'iphone 11 series') {
        // Count products with model in 11, 11pm, 11pro
        $countQuery .= " AND LOWER(model) IN ('11', '11pm', '11pro')";
    } elseif ($modelBrandFilter === 'iphone 12 series') {
        // Count products with model in 12, 12pm, 12pro
        $countQuery .= " AND LOWER(model) IN ('12', '12pm', '12pro')";
    } elseif ($modelBrandFilter === 'iphone 13 series') {
        // Count products with model in 13, 13pro, 13pm
        $countQuery .= " AND LOWER(model) IN ('13', '13pro', '13pm')";
    } elseif ($modelBrandFilter === 'iphone 14 series') {
        // Count products with model in 14, 14pro, 14pm
        $countQuery .= " AND LOWER(model) IN ('14', '14pro', '14pm')";
    } elseif ($modelBrandFilter === 'iphone 16 series') {
        // Count products with model in 16, 16e
        $countQuery .= " AND LOWER(model) IN ('16', '16e')";
    } elseif ($modelBrandFilter === 'iphone x series') {
        // Count products with model in x, xr, xs, xs max
        $countQuery .= " AND LOWER(model) IN ('x', 'xr', 'xs', 'xs max')";
    } elseif ($modelBrandFilter === 'iphone 6 series') {
        // Count products with model in 6, 6sp
        $countQuery .= " AND LOWER(model) IN ('6', '6sp')";
    } else {
...
$selectQuery = "SELECT * FROM products WHERE LOWER(product) = 'iphone'";
$selectParams = [];
if ($modelBrandFilter !== '' && $modelBrandFilter !== 'all models') {
    if ($modelBrandFilter === 'iphone 7 series') {
        // Select products with model in 7, 7p
        $selectQuery .= " AND LOWER(model) IN ('7', '7p')";
    } elseif ($modelBrandFilter === 'iphone 8 series') {
        // Select products with model in 8, 8p
        $selectQuery .= " AND LOWER(model) IN ('8', '8p')";
    } elseif ($modelBrandFilter === 'iphone 11 series') {
        // Select products with model in 11, 11pm, 11pro
        $selectQuery .= " AND LOWER(model) IN ('11', '11pm', '11pro')";
    } elseif ($modelBrandFilter === 'iphone 12 series') {
        // Select products with model in 12, 12pm, 12pro
        $selectQuery .= " AND LOWER(model) IN ('12', '12pm', '12pro')";
    } elseif ($modelBrandFilter === 'iphone 13 series') {
        // Select products with model in 13, 13pro, 13pm
        $selectQuery .= " AND LOWER(model) IN ('13', '13pro', '13pm')";
    } elseif ($modelBrandFilter === 'iphone 14 series') {
        // Select products with model in 14, 14pro, 14pm
        $selectQuery .= " AND LOWER(model) IN ('14', '14pro', '14pm')";
    } elseif ($modelBrandFilter === 'iphone 16 series') {
        // Select products with model in 16, 16e
        $selectQuery .= " AND LOWER(model) IN ('16', '16e')";
    } elseif ($modelBrandFilter === 'iphone x series') {
        // Select products with model in x, xr, xs, xs max
        $selectQuery .= " AND LOWER(model) IN ('x', 'xr', 'xs', 'xs max')";
    } elseif ($modelBrandFilter === 'iphone 6 series') {
        // Select products with model in 6, 6sp
        $selectQuery .= " AND LOWER(model) IN ('6', '6sp')";
    } else {
...
<head>
  ...
  <title>IPhone Products - Inventory System</title>
...
<body>
  <header class="flex flex-col py-4">
    ...
    <h1 class="text-6xl font-bold text-black-400 text-center" style="font-family: 'Times New Roman', serif;">IPHONES</h1>
  </header>
...
foreach ($modelBrands as $model) {
  ...
  if ($modelLower === '' || $modelLower === 'not available') continue;
    // Exclude individual 7, 7p, 8, 8p, 11, 11pm, 11pro, 12, 12pm, 12pro, 13, 13pro, 13pm, 14, 14pro, 14pm, 16, 16e, x, xr, xs, xs max models from buttons
    if (in_array($modelLower, ['7', '7p', '8', '8p', '11', '11pm', '11pro', '12', '12pm', '12pro', '13', '13pro', '13pm', '14', '14pro', '14pm', '16', '16e', 'x', 'xr', 'xs', 'xs max', '6s', '6sp'])) continue;
if ($modelLower === 'iphone 11 series') {
      // Show Iphone 11 series button with proper label
      ...
      echo '<a href="?' . $queryString . '" class="px-6 py-3 rounded-full border-2 border-blue-700 font-bold cursor-pointer transition-all duration-200 shadow-md m-1 ' . $activeClass . '" title="Filter by iPhone 11 Series" aria-label="Filter by iPhone 11 Series" style="user-select:none;">iPhone 11 Series</a>';
      continue;
  }
  if ($modelLower === 'iphone 8 series') {
      // Show Iphone 8 series button with proper label
      $urlParams['model_brand'] = $modelLower;
      $queryString = http_build_query($urlParams);
      $activeClass = ($modelLower === $currentModelBrand) ? 'bg-gradient-to-r from-blue-700 to-blue-900 text-white shadow-lg scale-105' : 'bg-gray-200 text-gray-900 hover:bg-blue-200 hover:scale-105';
      echo '<a href="?' . $queryString . '" class="px-6 py-3 rounded-full border-2 border-blue-700 font-bold cursor-pointer transition-all duration-200 shadow-md m-1 ' . $activeClass . '" title="Filter by iPhone 8 Series" aria-label="Filter by iPhone 8 Series" style="user-select:none;">iPhone 8 Series</a>';
      continue;
  }
  if ($modelLower === 'iphone 6 series') {
      // Show Iphone 6 series button with proper label
      $urlParams['model_brand'] = $modelLower;
      $queryString = http_build_query($urlParams);
      $activeClass = ($modelLower === $currentModelBrand) ? 'bg-gradient-to-r from-blue-700 to-blue-900 text-white shadow-lg scale-105' : 'bg-gray-200 text-gray-900 hover:bg-blue-200 hover:scale-105';
      echo '<a href="?' . $queryString . '" class="px-6 py-3 rounded-full border-2 border-blue-700 font-bold cursor-pointer transition-all duration-200 shadow-md m-1 ' . $activeClass . '" title="Filter by iPhone 6 Series" aria-label="Filter by iPhone 6 Series" style="user-select:none;">iPhone 6 Series</a>';
      continue;
  }
  if ($modelLower === 'iphone 12 series') {
      // Show Iphone 12 series button with proper label
      $urlParams['model_brand'] = $modelLower;
      $queryString = http_build_query($urlParams);
      $activeClass = ($modelLower === $currentModelBrand) ? 'bg-gradient-to-r from-blue-700 to-blue-900 text-white shadow-lg scale-105' : 'bg-gray-200 text-gray-900 hover:bg-blue-200 hover:scale-105';
      echo '<a href="?' . $queryString . '" class="px-6 py-3 rounded-full border-2 border-blue-700 font-bold cursor-pointer transition-all duration-200 shadow-md m-1 ' . $activeClass . '" title="Filter by iPhone 12 Series" aria-label="Filter by iPhone 12 Series" style="user-select:none;">iPhone 12 Series</a>';
      continue;
  }
  if ($modelLower === 'iphone 13 series') {
      // Show Iphone 13 series button with proper label
      $urlParams['model_brand'] = $modelLower;
      $queryString = http_build_query($urlParams);
      $activeClass = ($modelLower === $currentModelBrand) ? 'bg-gradient-to-r from-blue-700 to-blue-900 text-white shadow-lg scale-105' : 'bg-gray-200 text-gray-900 hover:bg-blue-200 hover:scale-105';
      echo '<a href="?' . $queryString . '" class="px-6 py-3 rounded-full border-2 border-blue-700 font-bold cursor-pointer transition-all duration-200 shadow-md m-1 ' . $activeClass . '" title="Filter by iPhone 13 Series" aria-label="Filter by iPhone 13 Series" style="user-select:none;">iPhone 13 Series</a>';
      continue;
  }
if ($modelLower === 'iphone 14 series') {
    // Show Iphone 14 series button with proper label
    $urlParams['model_brand'] = $modelLower;
    $queryString = http_build_query($urlParams);
    $activeClass = ($modelLower === $currentModelBrand) ? 'bg-gradient-to-r from-blue-700 to-blue-900 text-white shadow-lg scale-105' : 'bg-gray-200 text-gray-900 hover:bg-blue-200 hover:scale-105';
    echo '<a href="?' . $queryString . '" class="px-6 py-3 rounded-full border-2 border-blue-700 font-bold cursor-pointer transition-all duration-200 shadow-md m-1 ' . $activeClass . '" title="Filter by iPhone 14 Series" aria-label="Filter by iPhone 14 Series" style="user-select:none;">iPhone 14 Series</a>';
    continue;
}
if ($modelLower === 'iphone 16 series') {
    // Show Iphone 16 series button with proper label
    $urlParams['model_brand'] = $modelLower;
    $queryString = http_build_query($urlParams);
    $activeClass = ($modelLower === $currentModelBrand) ? 'bg-gradient-to-r from-blue-700 to-blue-900 text-white shadow-lg scale-105' : 'bg-gray-200 text-gray-900 hover:bg-blue-200 hover:scale-105';
    echo '<a href="?' . $queryString . '" class="px-6 py-3 rounded-full border-2 border-blue-700 font-bold cursor-pointer transition-all duration-200 shadow-md m-1 ' . $activeClass . '" title="Filter by iPhone 16 Series" aria-label="Filter by iPhone 16 Series" style="user-select:none;">iPhone 16 Series</a>';
    continue;
}
  if ($modelLower === 'iphone x series') {
      // Show Iphone x series button with proper label
      $urlParams['model_brand'] = $modelLower;
      $queryString = http_build_query($urlParams);
      $activeClass = ($modelLower === $currentModelBrand) ? 'bg-gradient-to-r from-blue-700 to-blue-900 text-white shadow-lg scale-105' : 'bg-gray-200 text-gray-900 hover:bg-blue-200 hover:scale-105';
      echo '<a href="?' . $queryString . '" class="px-6 py-3 rounded-full border-2 border-blue-700 font-bold cursor-pointer transition-all duration-200 shadow-md m-1 ' . $activeClass . '" title="Filter by iPhone X Series" aria-label="Filter by iPhone X Series" style="user-select:none;">iPhone X Series</a>';
      continue;
  }
  $urlParams['model_brand'] = $modelLower;
  $queryString = http_build_query($urlParams);
  ...
  echo '<a href="?' . $queryString . '" class="px-6 py-3 rounded-full border-2 border-blue-700 font-bold cursor-pointer transition-all duration-200 shadow-md m-1 ' . $activeClass . '" title="Filter by ' . htmlspecialchars($model) . '" aria-label="Filter by ' . htmlspecialchars($model) . '" style="user-select:none;">' . htmlspecialchars($model) . '</a>';
    ...
    <div class="grid">
      <?php
      if (count($products) === 0) {
          echo '<p class="col-span-full text-center text-gray-500 text-xl">No iPhone products found.</p>';
      } else {
          foreach ($products as $product) {
              ...
              // Display the friendly model name if it exists in the mapping
              $modelName = $product['model'];
              if (isset($modelDisplayNames[strtolower($modelName)])) {
                  $modelName = $modelDisplayNames[strtolower($modelName)];
              }
              echo '<p><strong>Model:</strong> ' . htmlspecialchars($modelName) . '</p>';
